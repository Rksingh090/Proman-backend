...
router.get('/',auth, async (req, res, next) => {
    try {
        ...
        const board1 = await Board.find({ userId: req.user })
        const board2 = await Board.find({ 'members.memberId': req.user })
...
// add board member on board id
router.patch('/addmember/:boardId', auth, async (req, res, next) => {
    const boardId = req.params.boardId
    const {memberId, memberName} = req.body
    try {
        const board = await Board.
            findOneAndUpdate({ _id: boardId }, {$push: {members: {memberId, memberName}}}, { new: true, runValidators: true })
        if (!board)
...
router.patch('/:id', auth, async (req, res, next) => {
    ...
    const allowedUpdates = ['name', 'description', 'Background']
